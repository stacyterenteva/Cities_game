...
int last_char_idx(const char* string)
{
    ...
    // TODO: use strings
    if (is_strings_equal(last_sym, "ь") && is_strings_equal(last_sym, "ъ") && is_strings_equal(last_sym, "ы") && is_strings_equal(last_sym, "й")) {
        return i - 1;
    }
    else {
    if (is_strings_equal(pre_last_sym, "ь") && is_strings_equal(pre_last_sym, "ъ") && is_strings_equal(pre_last_sym, "ы") && is_strings_equal(pre_last_sym, "й")) {
            return i - 2;
        }
        else {
            return i - 3;
